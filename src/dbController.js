...
const updateJsonData = (appointmentId, dynamicVarName, iUserValue) => {
 ...
 function getAppointmentDetailsByUserID(userId) {  
    return new Promise((resolve, reject) => {
    console.log(userId);
    const query = `SELECT Appointment_Type, POC_ID, Appointment_ID, DATE_FORMAT(Appointment_Date, '%Y-%m-%d') as Appointment_Date, Appointment_Time FROM appointments WHERE User_ID = ? AND Is_Active=1 AND(Appointment_Date > CURDATE() OR (Appointment_Date = CURDATE() AND Appointment_Time >= CURTIME()))`;  
    const connection = getConnection();
...
function getAppointmentDetailsByAppointmentId(appointmentId) {  
    return new Promise((resolve, reject) => {
    const query = `SELECT POC_ID,DATE_FORMAT(Appointment_Date, '%Y-%m-%d') as Appointment_Date, Appointment_Time FROM appointments WHERE Appointment_ID = ?`;  
    const connection = getConnection();  
...
async function increaseAvailableSlots(dynamicVariables) {  
    const pocId = dynamicVariables.Poc_ID;  
    const appointmentDate = dynamicVariables.Appointment_Date;  
    const appointmentTime = dynamicVariables.Appointment_Time;  
...
module.exports = { getAppointmentDetailsByAppointmentId,getAppointmentDetailsByUserID, increaseAvailableSlots,getMeetLink, getTemplateMessage, insertAppointment, updateAppointment, updateJsonData, updateAvailableSlots, insertUserData, getUserData, updateUserField, getClientID, getWelcomeMessage, getMainMenu, getFromList, getPocFromPoc, getAvailableDates, getAvailableTimes };